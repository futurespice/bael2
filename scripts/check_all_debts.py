"""
Показать ВСЕ магазины с долгом > 0, с полной разбивкой по заказам.

Запуск:
  docker compose exec -T web python manage.py shell < scripts/check_all_debts.py
"""
...
for store in stores:
    ...
    print(f"\n{'─' * 70}")
    print(f"  {store.name} (ID={store.id}, ИНН={store.inn})")
    ...
    calc = old_debt + new_debt
    if store.debt != calc:
        print(f"  ⚠️  store.debt ({store.debt}) != расчёт ({calc}), разница: {store.debt - calc:+}")
